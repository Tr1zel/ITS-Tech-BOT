...
async def check_registration(user_id: int):
    async with aiosqlite.connect('ITSTech.db') as db:
...
async def validate_group(study_group):
...
async def get_name_from_id(id: int):
    async with aiosqlite.connect('ITSTech.db') as db:

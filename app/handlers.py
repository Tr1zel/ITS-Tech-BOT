...
@router.message(Reg.waiting_study_group)
...
@router.message(F.text == 'Посмотреть свои баллы')
async def check_score(message: Message):
    user_id = message.from_user.id
    if await check_registration(user_id):
        name = await get_name_from_id(message.from_user.id)
...
@router.message(F.text == 'Посмотреть историю начислений баллов')
